        ...
        if(i.name==selected_file):
            ...
            if(len(df1.columns)>2):
                ...
                if(largest_variable=='a'):
                   ...
                   fore=best_model.predict(start=(pd.Series(df.index[-1])[0])+ pd.DateOffset(weeks=1),end=(pd.Series(df.index[-1])[0])+ pd.DateOffset(weeks=4),exog=df_exog[exog_var])
                   fore_df=pd.DataFrame(data=fore.reset_index())
                   fore_df=fore_df.rename(columns={'index':'dt','predicted_mean':'Quantity'})
                   a=pd.DataFrame(pred.reset_index().iloc[-1]).T.rename(columns={0:'Quantity'}).reset_index(drop=True)
                   dff=pd.concat([a,fore_df],ignore_index=True)
        ...
        if(i.name==selected_file):
            ...
            if(len(df.columns)>2):
               ...
               #dfa.rename(columns={'dt':'Date','Quantity':'Actual Production'},inplace=True)
                   ...
                   fore=best_model.predict(start=(pd.Series(df.index[-1])[0])+ pd.DateOffset(months=1),end=(pd.Series(df.index[-1])[0])+ pd.DateOffset(months=4),exog=df_exog[exog_var])
                   fore_df=pd.DataFrame(data=fore.reset_index())
                   fore_df=fore_df.rename(columns={'index':'dt','predicted_mean':'Quantity'})
                   a=pd.DataFrame(pred.reset_index().iloc[-1]).T.rename(columns={0:'Quantity'}).reset_index(drop=True)
                   dff=pd.concat([a,fore_df],ignore_index=True)
        ...
        if(i.name==selected_file):
            ...
            if(len(df.columns)>2):
               ...
               # df = pd.DataFrame()
                ...
                if(largest_variable=='x'):
                    ...
                    #dfa.rename(columns={'dt':'Date','Quantity':'Actual Production'},inplace=True)
                       ...
                       fore=best_model.predict(start=(pd.Series(df.index[-1])[0])+pd.DateOffset(days=1),end=(pd.Series(df.index[-1])[0])+ pd.DateOffset(days=4))
                       fore_df=pd.DataFrame(data=fore.reset_index())
                       fore_df=fore_df.rename(columns={'index':'dt','predicted_mean':'Quantity'})
                       a=pd.DataFrame(pred.reset_index().iloc[-1]).T.rename(columns={0:'Quantity'}).reset_index(drop=True)
                       dff=pd.concat([a,fore_df],ignore_index=True)
               ...
               #dfa.rename(columns={'dt':'Date','Quantity':'Actual Production'},inplace=True)
                   ...
                   fore=best_model.predict(start=(pd.Series(df.index[-1])[0])+ pd.DateOffset(days=1),end=(pd.Series(df.index[-1])[0])+ pd.DateOffset(days=4),exog=df_exog[exog_var])
                   fore_df=pd.DataFrame(data=fore.reset_index())
                   fore_df=fore_df.rename(columns={'index':'dt','predicted_mean':'Quantity'})
                   a=pd.DataFrame(pred.reset_index().iloc[-1]).T.rename(columns={0:'Quantity'}).reset_index(drop=True)
                   dff=pd.concat([a,fore_df],ignore_index=True)
